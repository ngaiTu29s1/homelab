...
ENV_REGEX = re.compile(r'^\.env(?:\.[A-Za-z0-9_-]+)?$')   # .env, .env.local, .env.prod ...
ENC_SUFFIX = ".enc"
...
def run_sops(args, inp_path: Path):
    ...
    cmd = ["sops", "--config", CONFIG_FILE, "--input-type", "dotenv", "--output-type", "dotenv"] + args + [str(inp_path)]
...
def fix_newlines():
    ...
    fixed = 0
    for coll in (list_env_files(), list_env_enc_files(), [Path(CONFIG_FILE)] if Path(CONFIG_FILE).exists() else []):
        for p in coll:
            try:
                raw = p.read_bytes()
